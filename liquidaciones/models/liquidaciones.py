        ...
        for rec in self:
            ...
            if total != 0:
                nuevas_lineas.append((0, 0, {
                    ...
                    'account_id': rec.cuenta_desajuste.id, #rec.facturas[0].account_id.id,
                    'date_maturity': rec.fecha,
                }))
            ...
            for linea in lineas:
                if not linea.reconciled:
                    for inver in invertidas:
                        if not inver.reconciled or not linea.partner_id.id or not linea.reconciled:
                            if linea.account_id.id == inver.account_id.id and linea.partner_id.id == inver.partner_id.id and (linea.debit+linea.credit) == (inver.debit+inver.credit)  :
                                par = linea | inver
                                if not inver.reconciled and not linea.reconciled:
                                    par.reconcile()
                indice += 1
