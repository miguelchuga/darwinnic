...
class ConciliacionBancariaXls(models.AbstractModel):
    _name = 'report.dev_bank_reconciliation.report_reconciliation_xls'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.merge_range('A3:I3', lines.name, format21)
  
        rec_row = 5
        
        sheet.write(rec_row, 1, 'Saldo Final Estado de Cuenta Bancario:', formatleft21)
        
        sheet.write(rec_row, 4, lines.bank_statement, money)
        
        rec_row += 2
        
        sheet.write(rec_row, 1, 'Conciliación Bancaria:', formatleft21)
        
        rec_row += 2
        
        sheet.write(rec_row, 1, '(-) Cheques y Notas Debito en Circulacion:', formatleft21)
        
        sheet.write(rec_row, 4, lines.less_unrepresented_amount, money)
        
        rec_row += 2
        
        sheet.write(rec_row, 1, '(+) Depositos y Creditos no Operados:', formatleft21)
        
        sheet.write(rec_row, 4, lines.deposit_not_credited_bank, money)
        ...
        sheet.write(rec_row, 4, lines.bank_account, money)
            
        rec_row += 2
        
        sheet.write(rec_row, 1, 'Diferencia:', formatleft21)
        
        sheet.write(rec_row, 4, lines.differance, money)
        ...
        self.env.cr.execute(sql, (lines.id,))
